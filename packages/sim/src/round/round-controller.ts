...
import { validatePlacement, type PlacementRejectReason } from '../placement/placement-validator.js'
...
function defaultTowerFields(): Pick<TowerEntity, 'targetingMode' | 'holdFire' | 'mvpStacks'> {
  return {
...
export class RoundController {
  ...
  towers: TowerEntity[] = []
  rocks: RockEntity[] = []
  pathCache: PathCache | null = null

  combatSession: CombatSession | null = null
  lastCombatSnapshot: CombatSnapshot | null = null
  pendingEvents: CombatEvent[] = []

  private readonly gemRoller: GemRoller
  ...
  tick(dt: number = 1 / SIM_HZ): SimTickResult {
  ...
  setTowerHoldFire(towerId: string, holdFire: boolean): void {
    const tower = this.towers.find((t) => t.id === towerId)
    if (tower) tower.holdFire = holdFire
  }
  ...
  upgradeGemChance(): UpgradeGemChanceResult {
    ...
    const maxLevel =
      this.content.gemProbability.levels.reduce((max, entry) => Math.max(max, entry.level), 1) ??
      MAX_GEM_CHANCE_LEVEL_V1
  ...
  resolveSelection(action: SelectionAction): void {
    ...
    const resolution = resolveSelection(this.content, this.candidates, action, () =>
      nextEntityId('entity'),
    )
  ...
  completeWave(): void {
    ...
    const retainCreeps =
      this.combatSession?.spawnerState.concurrent === true
  ...
  private startCombatSession(): void {
  ...
  private setTowersActive(active: boolean): void {
    for (const tower of this.towers) {
      tower.active = active
    }
  }
