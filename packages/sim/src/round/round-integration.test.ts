import { loadGameContent, type GameContent } from '@facet/content'
...
function contentWithTinyWave(): GameContent {
  const content = loadGameContent()
  return {
    ...content,
    waves: content.waves.map((w, i) =>
      i === 0
        ? {
            ...w,
            clearCount: 1,
...
describe('full round integration', () => {
  ...
  function placeFive(controller: RoundController): void {
    for (let gy = 24; gy < 120; gy += 4) {
      for (let gx = 24; gx < 120; gx += 4) {
        if (controller.phase !== 'placement') return
        const result = controller.placeCandidate(gx, gy)
        if (result.ok && controller.placementCharges === 0) return
      }
    }
