...
describe('RoundController', () => {
  ...
  function legalPlacementSlots(controller: RoundController): Array<{ gx: number; gy: number }> {
    const slots: Array<{ gx: number; gy: number }> = []
    for (let gy = 24; gy < 120; gy += 4) {
      for (let gx = 24; gx < 120; gx += 4) {
        const phase = controller.phase
        if (phase === 'selection') return slots
        if (phase !== 'placement') return slots
        const result = controller.placeCandidate(gx, gy)
        if (result.ok) {
          slots.push({ gx, gy })
        }
        if (controller.phase === 'selection') return slots
      }
    }
