...
function advanceAlongPolyline(
  ...
  distance: number,
): { pathIndex: number; distanceAlongSegment: number; worldPos: { x: number; y: number } } {
  let idx = pathIndex
  ...
  return {
    ...
    distanceAlongSegment: segmentLength(
      points[points.length - 2]?.x ?? end.x,
...
export function advanceGroundCreep(creep: CreepEntity, pathCache: PathCache, dt: number): boolean {
  const speed = creep.baseSpeed * effectiveSpeedMultiplier(creep)
  const points = pathCache.worldConcatenated
  if (points.length < 2) return true

  const result = advanceAlongPolyline(
    points,
    creep.pathIndex,
  ...
  creep.pathProgress = computePathProgress(
    leg.legIndex,
    leg.pathIndex,
    creep.distanceAlongSegment,
    pathCache.legStartIndex,
  )

  const atEnd =
    creep.pathIndex >= points.length - 2 &&
    creep.distanceAlongSegment >=
      segmentLength(
...
export function advanceFlyingCreep(
  creep: CreepEntity,
  ...
  creep.pathProgress =
    ...
    creep.distanceAlongSegment /
      Math.max(
        1,
        segmentLength(
          points[creep.pathIndex]?.x ?? 0,
          points[creep.pathIndex]?.y ?? 0,
          points[creep.pathIndex + 1]?.x ?? 0,
          points[creep.pathIndex + 1]?.y ?? 0,
        ),
      )

  const lastSeg = segmentLength(
    points[points.length - 2]!.x,
    points[points.length - 2]!.y,
    points[points.length - 1]!.x,
    points[points.length - 1]!.y,
...
export function initCreepPosition(
  creep: CreepEntity,
