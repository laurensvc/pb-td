...
function candidate(id: string, gemId: string, gx: number, gy: number): CandidateGem {
  const dash = gemId.lastIndexOf('-')
  return {
    id,
    gemId,
    type: gemId.slice(0, dash) as CandidateGem['type'],
    quality: gemId.slice(dash + 1) as CandidateGem['quality'],
    gx,
    gy,
  }
}
...
describe('SelectionResolver', () => {
  ...
  it('offers duplicate combine for matching pair', () => {
    ...
    expect(combine && combine.kind === 'duplicate-combine' ? combine.resultGemId : '').toBe(
      'ruby-flawed',
    )
  })
