...
function titleCaseQuality(quality: QualityTier): string {
  return quality.charAt(0).toUpperCase() + quality.slice(1)
}

export function generateGemDefinition(
  type: (typeof gemTypes)[number],
  quality: QualityTier,
): GemDefinition {
...
export function generateV1GemDefinitions(): GemDefinition[] {
  return gemTypes.flatMap((type) =>
    v1Qualities.map((quality) => generateGemDefinition(type, quality)),
  )
}
