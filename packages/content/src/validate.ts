...
import {
  assertProbabilityWeightsSumTo100,
  gemProbabilityTableSchema,
} from './schemas/gem-probability.js'
...
export function validateContent(
  ...
  if (!options.skipRouteValidation) {
    ...
    if (!routeResult.ok) {
      throw new Error(
        `Route ${routeResult.routeId} leg ${routeResult.leg.from} → ${routeResult.leg.to} disconnected on empty board`,
      )
    }
  }
