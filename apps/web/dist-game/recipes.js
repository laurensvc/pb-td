...
export function findMatchingRecipe(a, b) {
    for (const recipe of hybridRecipes) {
        const [i0, i1] = recipe.inputs;
        if (inputsMatch(i0, i1, a, b))
            return recipe;
    }
    return undefined;
}
export function listHybridRecipes() {
    return hybridRecipes;
}
export function describeRecipeInputs(recipe) {
    const [a, b] = recipe.inputs;
    return `${a.family} L${a.level} + ${b.family} L${b.level}`;
}
export function findRawRecipeMatches(rawGems) {
    if (rawGems.length < 5)
        return [];
