...
export const BuildPhasePanel = memo(function BuildPhasePanel({
  ...
  const ritualPhase = buildRitualPhase(snapshot.buildStep);
  const ritualSteps = [
    { id: 'place' as const, label: '1 · Place five' },
    { id: 'commit' as const, label: '2 · Commit one' },
    { id: 'ready' as const, label: '3 · Ready' },
  ];
  return (
    <section className="game-card build-phase-card">
      ...
      <p className="build-ritual-banner">
        {buildRitualHint(snapshot.buildStep, snapshot.rawGems.length)}
      </p>
      <div className="build-stepper" aria-label="Gem TD build ritual">
        {ritualSteps.map((step) => (
          <span
            ...
            className={
              ritualPhase === step.id
                ? 'build-step active'
                : ritualSteps.findIndex((candidate) => candidate.id === ritualPhase) >
                    ritualSteps.findIndex((candidate) => candidate.id === step.id)
                  ? 'build-step done'
                  : 'build-step'
            }
          >
            {step.label}
            {step.id === 'place' ? ` (${snapshot.rawGems.length}/${ROCKS_PER_PHASE})` : ''}
          </span>
        ))}
      </div>
      ...
      {snapshot.buildStep === 'rocks' && (
        <p className="hint">
          Ghost snaps to grid cells. Right-click a placed raw gem to undo. WASD / arrows pan · Q/E
          zoom · scroll wheel zooms under cursor.
        </p>
      )}
      {snapshot.buildStep === 'prospect' && (
        <p className="hint">
          Click a raw gem on the board or in Prospect to keep it. The other four become stone
          blocks. Right-click a gem to remove it and keep placing.
        </p>
      )}
      ...
      {snapshot.buildStep === 'ready' && (
        <p className="hint">Stones block the path. Merge gems, then Start Wave.</p>
      )}
    </section>
  );
});
