...
import { listHybridRecipes } from '../../../game/recipes';
import { gemFamilyBoardName } from '../../../game/gems';
        ...
        {BASE_GEM_FAMILIES.map((family) => {
          ...
          return (
            <div
              ...
              title={
                unlocked
                  ? `${def.name} great craft unlocked`
                  : `Complete quest to unlock ${def.name} L7`
              }
            >
...
export const RecipePanel = memo(function RecipePanel({ save }: { save: SaveState }) {
  const recipes = listHybridRecipes();
  return (
    <section className="game-card compact">
      <div className="card-header">
        <h2>Formulas</h2>
        <span className="hint">Pair recipes</span>
      </div>
      <p className="hint">
        Two raw gems with matching families and natural levels combine into the hybrid output. Order
        does not matter.
      </p>
      <ul className="recipe-list" aria-label="Hybrid gem formulas">
        {recipes.map((recipe) => {
            ...
            <li key={recipe.id} title={gemTooltip(save, recipe.output.family, recipe.output.level)}>
              <div className="recipe-row">
                <span
                  ...
                  style={{ '--tower-color': a.color } as CSSProperties}
                />
                <span
                  className="tower-icon"
                  style={{ '--tower-color': b.color } as CSSProperties}
                />
                <span className="recipe-arrow">→</span>
                <span
                  className="tower-icon"
                  style={{ '--tower-color': out.color } as CSSProperties}
                />
                <strong>{recipe.label}</strong>
              </div>
              <span className="hint">
                {gemFamilyBoardName(recipe.inputs[0].family)} L{recipe.inputs[0].level} +{' '}
                {gemFamilyBoardName(recipe.inputs[1].family)} L{recipe.inputs[1].level} → {out.name}{' '}
                L{recipe.output.level}
              </span>
            </li>
          );
        })}
