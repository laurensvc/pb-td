...
describe('pathNav hex', () => {
  ...
  it('builds nav data with ordered checkpoints', () => {
    expect(nav.pathCells.size).toBeGreaterThan(10);
    expect(nav.checkpoints).toHaveLength(4);
    expect(nav.checkpoints[0]).toEqual(nav.spawnCell);
    expect(nav.checkpoints[3]).toEqual(nav.goalCell);
    expect(nav.checkpointDistances).toHaveLength(4);
  ...
  it('steps enemies toward the next checkpoint', () => {
