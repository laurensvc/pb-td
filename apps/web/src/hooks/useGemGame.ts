...
export interface GemGameController {
...
export function useGemGame(): GemGameController {
  ...
  const bridge = useMemo<PhaserBridge>(
    () => ({
      ...
      step: (dt: number) => {
        ...
        const shouldPublish =
          uiDirty.current || state.status === 'running' || state.fxEvents.length > 0;

        if (shouldPublish && lastPublish.current >= PUBLISH_INTERVAL_S) {
          lastPublish.current = 0;
          const persist =
            state.status === 'running' || state.status === 'lost' || state.status === 'cleared';
          publish({ persist });
        }
      },
    }),
